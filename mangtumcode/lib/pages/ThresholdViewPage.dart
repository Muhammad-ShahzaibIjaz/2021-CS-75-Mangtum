...
class ThresholdViewPage extends StatefulWidget {
  final String userId;

  ThresholdViewPage({Key? key, required String userId})
      : userId = userId,
        super(key: key);

  factory ThresholdViewPage.create({Key? key, required String userId}) {
    return ThresholdViewPage(
      key: key,
      userId: userId,
    );
  }
  ...
  @override
  _ThresholdViewPageState createState() => _ThresholdViewPageState();
}

class _ThresholdViewPageState extends State<ThresholdViewPage> {
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (filteredProducts.isNotEmpty) {
            // Filter products based on threshold and quantity
            filteredProducts = filteredProducts.where((productData) {
              final threshold = productData['threshold'] ?? 0;
